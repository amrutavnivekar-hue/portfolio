...
export default function AdminDashboard() {
    ...
    ? [
        {
          ...
          value: metrics.totalSections ?? 0,
          sub: 'CMS sections',
          icon: '📄',
        },
        {
          label: 'Published',
          value: metrics.publishedSections ?? 0,
          sub: 'Live on site',
          icon: '✅',
        },
        {
          label: 'Drafts',
          value: metrics.draftSections ?? 0,
          sub: 'Pending publish',
          icon: '✏️',
        },
        {
          label: 'Revisions',
          value: metrics.totalRevisions ?? 0,
          sub: 'Total edits saved',
          icon: '🕓',
        },
        {
          label: 'Messages',
          value: metrics.totalMessages ?? 0,
          sub: `${metrics.unreadMessages ?? 0} unread`,
          icon: '✉️',
          href: '/admin/contact-forms',
          highlight: (metrics.unreadMessages ?? 0) > 0,
        },
        {
          label: 'Visitors (7d)',
          value: metrics.visitorsThisWeek ?? 0,
          sub: 'Page views',
          icon: '👁️',
        },
      ]
  ...
  return (
    <div className="space-y-6">
      ...
      {!metrics && !error && (
        <p className="text-sm opacity-40">Loading metrics...</p>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {statCards.map((card) => (
          ...
          >
            {card.highlight && (
              <span className="absolute top-3 right-3 h-2.5 w-2.5 rounded-full animate-pulse"
                style={{ background: 'var(--primary)' }} />
            )}
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs font-medium opacity-60 uppercase tracking-wider">{card.label}</p>
                <p className="mt-1 text-3xl font-bold" style={{ color: 'var(--primary)' }}>{card.value}</p>
                <p className="mt-0.5 text-xs opacity-50">{card.sub}</p>
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {/* Recent Activity */}
        <div className="rounded-xl p-5" style={{ background: 'var(--surface)', ...cardBorder }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold">Recent CMS Activity</h3>
          ...
          <div className="space-y-2">
            {metrics?.recentActivity?.length > 0
              ? metrics.recentActivity.slice(0, 6).map((log: any, idx: number) => (
                <div key={idx} className="rounded-lg p-3 text-sm" style={innerBorder}>
                  <div className="flex items-start justify-between gap-2">
                    <span className="font-medium" style={{ color: 'var(--primary)' }}>
                      {String(log.action || 'event')}
                    </span>
                    <span className="text-xs opacity-40 shrink-0">
                      {new Date(String(log.timestamp)).toLocaleDateString()}
                    </span>
                  </div>
        ...
        <div className="rounded-xl p-5" style={{ background: 'var(--surface)', ...cardBorder }}>
          ...
          {visitors?.topPages?.length > 0
            ? (
              <div className="space-y-2">
                {visitors.topPages.map((page: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-3">
                    <span className="text-xs opacity-40 w-4 text-right shrink-0">{idx + 1}</span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="h-1 rounded-full overflow-hidden"
                        ...
                        <div className="h-full rounded-full"
                          style={{
                            width: `${Math.round((page.count / visitors.topPages[0].count) * 100)}%`,
      ...
      {metrics?.unreadMessages > 0 && (
        <div className="rounded-xl p-4 flex items-center justify-between gap-4"
          style={{ background: 'color-mix(in srgb, var(--primary) 12%, var(--surface))', border: '1px solid var(--primary)' }}>
          <div>
